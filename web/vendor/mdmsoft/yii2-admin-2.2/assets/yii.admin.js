yii.admin = (function ($) {
    ...
    var pub = {
                ...
                setTimeout(function () {
                    ...
                    $.get(pub.searchUrl, data,
                        function (r) {
                            ...
                            if (r.Permissions) {
                                var $group = $('<optgroup label="Permisos">');
                                $.each(r.Permissions, function () {
                                    $('<option>').val(this).text(this).appendTo($group);
                                });
                                $group.appendTo($list);
                           ...
                           if (r.Routes) {
                                var $group = $('<optgroup label="Acciones">');
        ...
        addChild2: function (action) {
            ...
            $.post(pub.assignUrl, params,
                function () {
                    pub.searchRole2('avaliable', true);
                    pub.searchRole2('assigned', true);
                });
        },
        
        searchRole2: function (target, force) {
            if (!_onSearch || force) {
                _onSearch = true;
                var $inp = $('#search-' + target);
                setTimeout(function () {
